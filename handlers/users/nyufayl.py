...
@dp.message_handler(Text(startswith='https://ww.insagram.com/'))
async def send_media(message:types.Message):
    link = message.text
    data = instadownloader(link=link)
    if data == 'Bad':
        await message.answer('Bu URL manzili oqali hech narsa topilmadi!')
    else:
        if data['type'] == 'image':
            ...
            await message.answer('Bu URL manzili orqali hech narsa topolmadik!')
